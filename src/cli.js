...
export async function cli(args){
    ...
    .then((data) => {
        if(JSON.parse(data).size > 0){
            ...
            processApexLogs(JSON.parse(data).records);
        }
    })
}   

function getApexLogIds(){
    return new Promise((resolve, reject) => {
        ...
        if(sessionInformation.debug) console.log('getApexLogIds options: ' , options)

        let url2GetApexLogIds = sessionInformation.instanceUrl + apexLogIdsQueryUrl + sessionInformation.queryWhere;

        if(sessionInformation.debug) console.log('getApexLogIds url2GetApexLogIds: ', url2GetApexLogIds);

        const req = http.request(encodeURI(url2GetApexLogIds), options, function(res) {
            if(sessionInformation.debug) console.log('getApexLogIds response code: ' + res.statusCode);
            ...
            res.on('end', function() {
                ...
                try {
                    ...
                    if(sessionInformation.debug) console.log('getApexLogIds error: ' , e);
                    reject(e);
                }
...
function processApexLogs(apexLogList){
    for(const apexLogIndex in apexLogList){
        ...
        if(sessionInformation.debug) console.log('processApexLogs options: ' , options);
        ...
        let fileName = 
            ...
            apexLog.Operation.replace('/','') + '-' + 
            apexLog.LastModifiedDate + '-' + 
            apexLog.LogUser.Name + '.log';
        
        http.request(completeUrl,options, function(res) {
            if(sessionInformation.debug) console.log('processApexLogs response code: ' + res.statusCode);

            if (res.statusCode < 200 || res.statusCode >= 300) {
                return reject(new Error('statusCode=' + res.statusCode));
            }
            res.setEncoding('utf8');
            res.on('data', function (apexLogBody) {
                saveApexLog(fileName, apexLogBody);
            });
        }).end();
    }
}
